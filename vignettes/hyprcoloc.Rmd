...
The HyPrColoc variant specific priors require the specification of two parameters: "prior.1" and "prior.c". The prior parameter "prior.1" denotes the probability that a snp is associated with a single trait (denoted "p" in the HyPrColoc manuscript and "p1" in COLOC). We refer the prior parameter "prior.c" as the 'conditional colocalization prior'. Note we additionally allow users to set the prior probability that a variant is associated with two traits, i.e. "p12" in COLOC. This is denoted as "prior.12" in HyPrColoc. Users who set a value for "prior.12", will automatically compute a value for "prior.c" via $prior.c = \frac{prior.12}{prior.1 + prior.12}$, as in COLOC.

For convenience we introduce the parameter "prior.2", where $prior.2 = 1 - prior.c$. From this, we can express the prior probability that a variant is assiciated with more than a pair of traits: $1-(prior.2)$ is the prior probaility that a snp is associated with an additonal trait given that it is associated with one trait; $1-(prior.2)^2$ is the prior probability that the snp is associated with a third trait given it is associated with two other traits; $1-(prior.2)^3$ is the prior probability that the snp is associated with a fourth trait given three and so on... By default $prior.1 = 10^{-4}$, hence the prior probability that any snp is a ssociated with a single trait is 1 in 10000 (matching "p1" in COLOC). Additionally, by default $prior.c = 0.02$, i.e. $prior.2 = 0.98$. Using the matching formula above, it follows that $prior.12\approx 0.02$ which is smaller (i.e. more conservative) than the recommended default $p12 = 0.05$ by Wallace (Plos Genetics, 2020). Hence, the prior probability that a snp is associated with a second trait, conditional on association with one trait, is 1 in 50; the prior probaility that it is associated with a third trait given association with two other traits is around 1 in 25; and so on... Notice that the marginal probability that a snp is associated with an increasing number of traits is always small and monotonic decreasing, e.g. $\frac{1}{10^{4}*50*25*\dots}$.   

When using the variant specific prior, results tend to be most sensitive to the choice of $prior.c$ as this controls the prior probability of each additional colocalized trait. Hence, we focus our sensitivity analysis on varying this parameter. We consider the range of values $prior.c \in\{0.05, 0.02, 0.01, 0.005\}$, equivalently $prior.2 \in\{0.95, 0.98, 0.99, 0.995\}$, meaning that the probability of a snp being associated with a second trait given its association with one trait can be: (i) 1 in 20, i.e. $prior.c=0.05$; (ii) 1 in 50, i.e. $prior.c=0.02$; (iii) 1 in 100, i.e. $prior.c=0.01$; (ii) 1 in 200, i.e. $prior.c=0.005$, 
